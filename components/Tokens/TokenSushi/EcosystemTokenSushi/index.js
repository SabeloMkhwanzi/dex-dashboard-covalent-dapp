...
export default function EcosystemTokenSushi() {
  // use React-Query to fetch Covalent API
  const { data, error, isFetching } = useQuery(["ecosystem17"], async () => {
  ...
  const chainItems = data?.data?.items || []; // Ensure chainItems is at least an empty array
