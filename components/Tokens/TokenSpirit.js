...
import EcosystemTokensSpirit from '../../components/Ecosystem/EcosystemTokensSpirit'

import { useGetTokensQuery } from '/services/tokensCovalentApiSpirit'
        ...
        <TableBody>
          {chainItems.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((items) => (
            <Tr key={items.chain_id}>
              ...
              <Td color={useColorModeValue("white", "gray.200")}>${millify(items.total_volume_24h_quote)}</Td>
              <Td color={useColorModeValue("white", "gray.200")}>${millify(items.quote_rate)}</Td>
